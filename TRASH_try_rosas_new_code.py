...
#!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def draw_cards(self):
        ...
        # Show cards only if game is not paused 
        if not pause_switch: 
            for i, (card_x, card_y) in enumerate(positions):
                ...
                if card_y + CARD_HEIGHT > 0 and card_y < DISPLAY_HEIGHT:
                    ...
                    elif i in self.computer_last_set_indices and time.time() < self.computer_pause_end and not pause_switch:
...
# VANAF HIER WERKT HET 
    ...
    def handle_click(self, pos):
        if not pause_switch:    ## ELIZA WAS HERE 
    ...
    def handle_keyboard_input(self,event):
        ...
        if pygame.K_0 <= event.key <= pygame.K_9 and not pause_switch:
            ...
            if mods & pygame.KMOD_SHIFT and not pause_switch:
...
#!!!!!!!!!!!!!!!
    ...
    def run(self):
        ...
        while running:
            ...
            #    #pygame.display.flip()
            #    print(pause_switch)
            #elif not pause_switch:
            #    print(pause_switch)

            if not self.game_over: #and not pause_switch:
                
                self.update_timer() # updates the timer and checks if time is up
                self.draw_cards() # draws the cards on the screen
            #elif not self.check_game_over and pause_switch:
            #    self.update_timer()
            #    pygame.draw.rect(self.screen, (153, 50, 204), (40, 70, 950, 400))
            #    pygame.display.flip()                
            else:
                #self.message = "GAME IS OVER IDIOT"
