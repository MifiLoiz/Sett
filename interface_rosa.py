...
class SetGame:
    ...
    def update_timer(self):
        ...
        if self.time_remaining <= 0:
            ...
            self.timer_start = time.time()  # resets the timer
            self.time_remaining = TIMER_DURATION  # resets the time remaining
            self.selected_indices.clear()
            self.hint_used = False  # resets hint usage after time is up
            self.hint_cards = []  # clears hint cards
...
# there isnt an end if the deck is empty (last sets can be found and removed, after, game over)
